...
#define MQTT_CLIENT_PRO        8
#define MQTT_CLIENT_STK_SIZE   300
OS_STK MQTT_CLIENT_TASK_STK[MQTT_CLIENT_STK_SIZE];

#define MQTT_CLIENT_SEND_PRO        9
#define MQTT_CLIENT_SEND_STK_SIZE   300
OS_STK MQTT_CLIENT_SEND_TASK_STK[MQTT_CLIENT_SEND_STK_SIZE];
...
void MqttConnectThread(void *pvParameters)
{
	  uint32_t curtick;
...
MQTT_START: 
    ...
    //获取当前滴答，作为心跳包起始时间
        ...
        if((OSTimeGet() - curtick) >(KEEPLIVE_TIME/2*1000))
        {
            curtick = OSTimeGet();
            //判断是否有数据交换
...
void MqttClientSendThread(void *param)
{
  int32_t ret;
  uint8_t no_mqtt_msg_exchange = 1;
  uint32_t curtick;
...
//        res = cJSON_Update(cJSON_Data,HUM_NUM,&b);
        res = CjsonUpdate(cJSON_Data,TEMP_NUM,&a);
        res = CjsonUpdate(cJSON_Data,HUM_NUM,&b);
...
//            vPortFree(p);
            //如果内存释放有问题的话，就把这类函数修改为申请内部内存，释放外部内存，然后测试
            mymexfree(p);
            p = NULL;
        }
        else
...
INT8U MqttClientThreadInit(void) {
  INT8U res;
  OS_CPU_SR cpu_sr;
  
  OS_ENTER_CRITICAL();
  
  res = OSTaskCreate(MqttConnectThread, (void *)0, (OS_STK *)&MQTT_CLIENT_TASK_STK[MQTT_CLIENT_STK_SIZE - 1], MQTT_CLIENT_PRO);
  res = OSTaskCreate(MqttClientSendThread, (void *)0, (OS_STK *)&MQTT_CLIENT_SEND_TASK_STK[MQTT_CLIENT_SEND_STK_SIZE - 1], MQTT_CLIENT_SEND_PRO);
