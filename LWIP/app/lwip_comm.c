...
////	ram_heap=mymalloc(SRAMIN,(My_MEM_SIZE_ALIGNED + (2U * LWIP_MEM_ALIGN_SIZE(sizeof(struct mem)))));	//为ram_heap申请内存 
...
//	if(!memp_memory||!ram_heap||!TCPIP_THREAD_TASK_STK||!LWIP_DHCP_TASK_STK||!LWIP_DM9000_INPUT_TASK_STK)//有申请失败的
  if(!TCPIP_THREAD_TASK_STK||!LWIP_DHCP_TASK_STK||!LWIP_DM9000_INPUT_TASK_STK)//有申请失败的
	{
		lwip_comm_mem_free();
		return 1;
	}
